...
(defvar byte-compile-emit-ir nil)
...
(byte-defop 184 0 byte-if)
(byte-defop 185 0 byte-else)
(byte-defop 186 0 byte-end)

(byte-defop 187 0 byte-loop)
(byte-defop 188 0 byte-br)
(byte-defop 189 -1 byte-br-if)
(byte-defop 190 0 byte-block)
...
(defconst byte-br-ops '(byte-br byte-br-if)
  "")
       ...
       (t
        ...
        (cond ((memq op byte-goto-ops)
              ...
              ((or (and (>= opcode byte-listN)
                   ...
                   (memq opcode (list byte-br-if byte-br)))
  ...
  (let (vars)
	...
	(cond ((or (not (symbolp arg))
		 ...
		 (error "Garbage following &rest VAR in lambda-list"))
               (when byte-compile-emit-ir
                 (error "&rest arguments cannot be JIT compiled"))
...
(defun byte-compile-quo (form)
  (let ((len (length form)))
    (cond ((< len 2)
	  ...
	  ((and (= len 3) (not byte-compile-emit-ir))
...
(defvar byte-compile--block-depth -1)

(defmacro byte-compile-ir-block (block-start-op block-end-op &rest form)
  `(progn
     (byte-compile-out ,block-start-op)
     (cl-incf byte-compile--block-depth)
     ,@form
     (byte-compile-out ,block-end-op)
     (cl-decf byte-compile--block-depth)))

(defun byte-compile-ir-if (form)
  (byte-compile-form (nth 1 form))
  (let ((clause (nth 1 form))
        (old-depth byte-compile-depth))

    (cl-decf byte-compile-depth)
    (byte-compile-out 'byte-if)
    (cl-incf byte-compile--block-depth)
    (byte-compile-maybe-guarded clause
      (byte-compile-form (nth 2 form) nil))
    (cl-assert (>= byte-compile-depth old-depth))
    (byte-compile-discard (- byte-compile-depth old-depth) t)
    (byte-compile-out 'byte-else)

    (cl-decf byte-compile-depth)

    ;; else...end
    (byte-compile-maybe-guarded (list 'not clause)
      (byte-compile-body (cdr (cdr (cdr form))) nil))
    (cl-assert (>= byte-compile-depth old-depth))
    (byte-compile-discard (- byte-compile-depth old-depth) t)
    (cl-decf byte-compile--block-depth)
    (byte-compile-out 'byte-end)))

(defun byte-compile-if (form)
  (if byte-compile-emit-ir
      (byte-compile-ir-if form)
    (byte-compile-form (car (cdr form)))
...
(defun byte-compile-cond-form-to-if (clauses)
  (let (if-form)
    (dolist (clause (butlast (reverse clauses) 1) if-form)
      (setq if-form `(if ,(car clause) (progn ,@(cdr clause)) ,if-form)))))

(defun byte-compile-ir-cond (clauses)
  (byte-compile-ir-if (byte-compile-cond-form-to-if clauses)))

(defun byte-compile-cond (clauses)
  (or (and byte-compile-cond-use-jump-table (not byte-compile-emit-ir)
           (byte-compile-cond-jump-table clauses))
      (if byte-compile-emit-ir
          (byte-compile-ir-cond clauses)
...
(defun byte-compile-ir-and (form)
  (byte-compile-ir-if (byte-compile-and-form-to-if form)))

(defun byte-compile-and-form-to-if (form)
  (let ((clauses (cdr form)))
    (byte-compile-and-form-to-if-1 clauses)))

(defun byte-compile-and-form-to-if-1 (clauses)
  (if clauses
      `(if ,(car clauses)
	   ,(if (= (length clauses) 1)
		(car clauses)
	      (byte-compile-and-form-to-if-1 (cdr clauses)))
	 nil)
    nil))
...
(defun byte-compile-or (form)
  (let ((wintag (byte-compile-make-tag))
	...
	(byte-compile-form-do-effect nil)
      (if byte-compile-emit-ir
          (byte-compile-ir-or form)
        (byte-compile-or-recursion args wintag)))))

(defun byte-compile-ir-or (form)
  (byte-compile-ir-if (byte-compile-or-form-to-if form)))

(defun byte-compile-or-form-to-if (form)
  (let ((clauses (cdr form)))
    (byte-compile-or-form-to-if-1 clauses)))

(defun byte-compile-or-form-to-if-1 (clauses)
  (if clauses
      `(if ,(car clauses)
	   ,(car clauses)
	 ,(byte-compile-or-form-to-if-1 (cdr clauses)))
    nil))
...
(defun byte-compile-ir-br (depth)
  (cl-assert (> depth -1))
  (byte-compile-out 'byte-br depth))

(defun byte-compile-ir-br-if (depth)
  (cl-assert (> depth -1))
